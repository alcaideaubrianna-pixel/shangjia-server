...
> 目标：同一镜像可按 API、Worker、Runtime 三种角色部署，并支持平台横向扩容；默认 `all` 启动方式仅用于本地开发或单实例部署。

### 运行角色与启动边界

- **API（web）**：只提供 HTTP、Webhook、WebSocket 等对外入口，不启动采集监听、轮询、定时调度或业务队列 Worker
- **Worker（worker）**：只消费可横向扩容的队列任务，不持有 Telegram Bot polling、用户账号 session 等独占长连接
- **Runtime（runtime）**：运行 Telegram Bot 管理、用户账号监听、Scheduler、Supervisor 等需要固定归属或租约的后台进程
- 所有部署必须显式指定启动命令和运行角色，禁止生产环境依赖二进制默认的 `all` 命令
- 插件路由可以在 API 实例注册，但插件后台 Runtime 必须先经过统一角色判断，禁止在插件 `Start()` 中无条件启动后台循环
- 运行角色判断必须放在主模块公共能力中，插件之间禁止互相 import 角色配置
- 新增后台 Runtime 时，必须在代码和部署文档中声明允许运行的角色及扩容方式

### 定时任务与后台循环

- 多实例 Cron、Scheduler、Recovery、Cleaner 必须使用 Redis 分布式锁、数据库租约或原子抢占，GoFrame `gcron.AddSingleton` 只能保证单进程内单例，不能作为集群锁
- 只允许一个实例执行的任务默认放入 Runtime 角色；即使固定单副本，也必须尽量实现分布式锁，避免滚动发布期间新旧实例重叠执行
- 后台循环必须监听 `ctx.Done()`，插件 `Stop()` 必须主动取消并等待退出，禁止无法优雅停止的永久 goroutine
- 启动恢复任务必须幂等，不得因多个实例同时启动而重复创建订单、重复推送、重复采集或重复迁移数据
- 数据库结构迁移、历史回填和大表索引只能通过显式部署 Job 或手动命令执行，禁止由每个 API/Worker 实例在请求或启动阶段并发执行 DDL

### 队列与幂等

- Worker 可以横向扩容，但消费者必须使用业务唯一键、状态机或数据库条件更新保证幂等
- 队列任务必须明确成功确认、失败重试、超时恢复和死信处理；禁止出队后进程崩溃即永久丢失任务
- 生产业务优先使用支持 ACK、重试和任务检查的队列实现；Redis `LPOP` 类消费只能用于允许丢失的非关键日志或通知
- HTTP 请求中禁止同步执行耗时采集、媒体处理和批量推送，应先落库再入队，由 Worker 异步处理
- 重试不得直接重复执行不可逆操作；Telegram 发送、支付回调、会员开通等必须以业务单号或任务 ID 去重

### Telegram 独占资源

- 同一个 Telegram Bot Token 的 polling 同一时间只能由一个 Runtime 实例持有；Webhook 可由多个 API 实例接收，但 Update 必须通过共享队列分发并幂等处理
- 同一个 Telegram 用户账号 session 同一时间只能由一个 Runtime 实例持有，必须通过数据库或 Redis 租约分配，租约需包含续期、过期回收和进程退出释放
- Telegram session 必须以数据库或共享存储为事实来源，容器本地 session 文件只能作为临时兼容缓存，不能作为唯一登录态
- Bot 菜单同步、`setWebhook`、账号恢复、监听重连等控制面操作只能由 Runtime 角色执行，禁止每个 API/Worker 副本重复执行
- Telegram 限流、FloodWait 和网络重试必须在共享任务状态中记录，不能只保存在单实例内存

### 状态、缓存与文件

- 禁止使用进程内 `map`、`sync.Map`、本地缓存作为业务唯一状态；交互会话、任务进度、锁和去重数据必须保存到数据库或 Redis，并设置明确过期时间
- 修改配置或权限后必须通过 Redis Pub/Sub、版本号或主动失效机制同步所有实例，禁止只清理当前实例缓存
- WebSocket 在线连接可以保存在本机，但跨实例广播、用户定向消息和在线状态聚合必须通过 Redis Pub/Sub 或共享消息总线
- 上传文件、媒体缓存和生成文件不得依赖容器持久磁盘；原始文件必须落 OSS/S3 等对象存储，本地目录只允许保存可随时重建的临时文件
- 临时文件必须使用系统临时目录并在成功、失败和取消路径清理；不得假设下一次请求会命中同一个实例

### 健康检查与优雅退出

- 每个可部署角色都必须提供 `/healthz` 和 `/readyz`：`healthz` 只表示进程存活，`readyz` 检查该角色的必要依赖
- API 的 `readyz` 至少检查数据库和 Redis；Worker 检查队列依赖；Runtime 检查数据库、Redis 和租约基础能力，但不应要求所有 Telegram 账号始终在线
- 健康检查不得执行 Telegram 外部请求、批量查询或其他高延迟操作
- 所有角色必须正确处理 `SIGTERM`：停止接收新任务、取消后台循环、释放账号租约、关闭 Bot/Telegram 客户端，并在平台终止宽限期内退出
- 滚动发布时必须允许新旧实例短暂共存，任何单例逻辑都不能只依赖“部署副本数为 1”

### CI/CD 与部署检查

- CI 只负责测试、构建和推送不可变镜像，部署、扩缩容和回滚由托管平台控制
- 镜像标签必须有可追溯的提交 SHA；制品库需自动清理历史标签，但禁止删除当前生产和回滚版本
- 上线前必须确认：数据库迁移已完成、角色命令已显式配置、Runtime 单例/租约有效、健康检查通过、对象存储可用、队列无大量积压
- 扩容前必须验证任务幂等、锁超时、优雅退出和实例重启恢复；未经验证的 Runtime 不得直接增加副本
