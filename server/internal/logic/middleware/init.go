...
func (s *sMiddleware) Ctx(r *ghttp.Request) {
	...
	// 链路追踪
	if g.Cfg().MustGet(r.Context(), "telemetry.switch").Bool() || g.Cfg().MustGet(r.Context(), "jaeger.switch").Bool() || g.Cfg().MustGet(r.Context(), "apm.switch").Bool() {
...
func (s *sMiddleware) IsExceptLogin(ctx context.Context, appName, path string) bool {
	if appName == consts.AppApi && gstr.HasPrefix(path, "/pay/notify/") {
		return true
	}
