...
func (s *sSysConfig) ensureEpayConfig(ctx context.Context) (err error) {
	defaults := []struct {
		...
		{key: "payGMPayToken", name: "GMPay 默认代币", typ: consts.ConfigTypeString, value: "", sort: 970, tip: "与 network 同时配置；两者留空时由结账页选择支付方式"},
		{key: "payGMPayNetwork", name: "GMPay 默认网络", typ: consts.ConfigTypeString, value: "", sort: 980, tip: "与 token 同时配置；两者留空时由结账页选择支付方式"},
