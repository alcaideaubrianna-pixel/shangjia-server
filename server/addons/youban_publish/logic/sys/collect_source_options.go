...
func (s *sSysPublish) AdminCollectSourceOptions(ctx context.Context, keyword string, page, perPage int) (list []*sysin.CollectSourceOptionModel, totalCount int, err error) {
	...
	base := g.DB().Model(publishCollectEventTable+" e").Safe().Ctx(ctx).
	...
	if keyword = strings.Trim(strings.TrimSpace(keyword), "@"); keyword != "" {
		like := "%" + keyword + "%"
		base = base.Where("(c.channel_title LIKE ? OR c.channel_username LIKE ? OR bc.chat_title LIKE ? OR bc.chat_username LIKE ? OR e.source_chat_id LIKE ? OR s.title LIKE ?)", like, like, like, like, like, like)
	}
	if totalCount, err = countCollectSourceOptionGroups(ctx, base); err != nil {
		return nil, 0, gerror.Wrap(err, "统计采集频道失败")
	}
	mod := base.Fields("e.source_id,e.source_chat_id,MAX(s.title) AS title,MAX(s.source_username) AS source_username,COALESCE(MAX(c.channel_title),MAX(bc.chat_title)) AS channel_title,COALESCE(MAX(c.channel_username),MAX(bc.chat_username)) AS channel_username").
