...
func (s *sSysPublish) BotProfileSearch(ctx context.Context, in *sysin.BotProfileSearchInp) (list []*sysin.NoteModel, totalCount int, err error) {
	...
	profileIn.TenantId = in.TenantId
	profileIn.AccountId = in.AccountId
	profileIn.Keyword = strings.TrimSpace(in.Keyword)
	profileIn.Status = in.Status
...
func (s *sSysPublish) BotProfileImageSearch(ctx context.Context, in *sysin.BotProfileImageSearchInp) (list []*sysin.NoteModel, totalCount int, err error) {
	...
	searchIn := &in.ProfileImageSearchInp
	normalizeProfileImageSearchInput(searchIn)
	profileIds, totalCount, err := s.findSimilarProfileIdsByPHash(ctx, queryHash, searchIn, nil)
	if err != nil {
		return nil, 0, err
	}
	if len(profileIds) == 0 {
