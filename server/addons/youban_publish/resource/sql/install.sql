...
CREATE TABLE IF NOT EXISTS `hg_youban_publish_collect_dedupe_entry` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ybp_collect_dedupe_entry` (`tenant_id`,`account_id`,`channel_id`,`layer`,`signature`,`item_total`,`signature_count`),
...
CREATE TABLE IF NOT EXISTS `hg_youban_publish_collect_dedupe_source` (
  ...
  UNIQUE KEY `uk_ybp_collect_dedupe_source` (`entry_id`,`dispatch_id`),
...
CREATE TABLE IF NOT EXISTS `hg_youban_publish_collect_dispatch` (
  ...
  `source_type_snapshot` varchar(32) NOT NULL DEFAULT '' COMMENT '采集来源类型快照',
  `source_name_snapshot` varchar(255) NOT NULL DEFAULT '' COMMENT '采集来源名称快照',
  `source_username_snapshot` varchar(128) NOT NULL DEFAULT '' COMMENT '采集来源用户名快照',
  `source_chat_id_snapshot` varchar(128) NOT NULL DEFAULT '' COMMENT '采集来源Chat ID快照',
  `source_message_id_snapshot` bigint(20) NOT NULL DEFAULT '0' COMMENT '采集来源消息ID快照',
...
CREATE TABLE IF NOT EXISTS `hg_youban_publish_cloud_resource_usage` (
  ...
  `provider` varchar(32) NOT NULL DEFAULT 'legacy' COMMENT '服务来源',
  ...
  `updated_at` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ybp_cloud_usage_daily` (`tenant_id`,`account_id`,`resource_type`,`provider`,`scene`,`usage_date`),
