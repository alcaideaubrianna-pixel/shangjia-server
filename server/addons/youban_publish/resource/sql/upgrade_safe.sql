...
ALTER TABLE `hg_youban_publish_cloud_resource_usage` ADD COLUMN IF NOT EXISTS `provider` varchar(32) NOT NULL DEFAULT 'legacy' COMMENT '服务来源' AFTER `resource_type`;
ALTER TABLE `hg_youban_publish_cloud_resource_usage` DROP INDEX `uk_ybp_cloud_usage_daily`, ADD UNIQUE INDEX `uk_ybp_cloud_usage_daily` (`tenant_id`,`account_id`,`resource_type`,`provider`,`scene`,`usage_date`);
