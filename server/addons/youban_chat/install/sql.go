...
var mysqlBusinessSqlFiles = []string{
	"addons/youban_chat/resource/sql/install.sql",
	"addons/youban_chat/resource/sql/menu.sql",
}

var pgsqlBusinessSqlFiles = []string{
	"addons/youban_chat/resource/sql/install.pgsql.sql",
	"addons/youban_chat/resource/sql/menu.pgsql.sql",
}
...
func execBusinessSql(ctx context.Context) (err error) {
	for _, file := range businessSqlFiles() {
		if err = execSqlFile(ctx, file); err != nil {
			return gerror.Wrapf(err, "执行悦伴聊天 SQL 失败：%s", file)
		}
	}
	return nil
}
