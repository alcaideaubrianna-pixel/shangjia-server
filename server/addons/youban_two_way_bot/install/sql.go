...
var mysqlSqlFiles = []string{
	"addons/youban_two_way_bot/resource/sql/install.sql",
	"addons/youban_two_way_bot/resource/sql/upgrade.sql",
	"addons/youban_two_way_bot/resource/sql/menu.sql",
}

var pgsqlSqlFiles = []string{
	"addons/youban_two_way_bot/resource/sql/install.pgsql.sql",
	"addons/youban_two_way_bot/resource/sql/upgrade.pgsql.sql",
	"addons/youban_two_way_bot/resource/sql/menu.pgsql.sql",
}
...
func execSql(ctx context.Context) (err error) {
	for _, file := range sqlFiles() {
		if err = execSqlFile(ctx, file); err != nil {
			return gerror.Wrapf(err, "执行双向机器人 SQL 失败：%s", file)
		}
	}
	return nil
}
	...
	for _, r := range content {
		...
		case ';':
			if quote == 0 {
				...
				if part != "" {
					list = append(list, strings.TrimSuffix(part, ";"))
				}
				builder.Reset()
			}
		}
