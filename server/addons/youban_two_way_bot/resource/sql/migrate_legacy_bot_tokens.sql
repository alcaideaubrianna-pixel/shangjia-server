...
WHERE publish_bot.`deleted_at` IS NULL
...
SELECT
  ...
  TRIM(LEADING '@' FROM TRIM(legacy.`bot_username`)),
  TRIM(legacy.`bot_token`),
  '由旧版双向机器人迁移',
...
LEFT JOIN `hg_youban_two_way_bot_bot` newer
  ...
  AND (
    newer.`status` > legacy.`status`
    OR (newer.`status` = legacy.`status` AND newer.`id` > legacy.`id`)
  )
